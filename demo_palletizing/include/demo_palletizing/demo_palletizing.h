...
private:
    ...
    //**map holding, for each actuated joint of the mainpulator chain, the joint link and the join root link */
    std::map<std::string, std::pair<std::string, std::string> > manipulator_joint_map_;
