...
//            boost::shared_ptr<TaskLink> t_obj(new TaskLink(i,chain,collision_root,joints_)); //create a new task object
...
//            task_manager_.addTaskLink(t_obj); //store the new object in the task manager
//        }
...
//            boost::shared_ptr<TaskLink> t_obj(new TaskLink(task_manager_.getValidTaskLinkId(), chain,root,joints_));
...
//            t_obj->computeKinematics();
//            task_manager_.addTaskLink(t_obj);

//            //generate a joint limits task
//            boost::shared_ptr<std::vector<TaskLink> > t_objs(new std::vector<TaskLink>);
...
bool HQPVelocityController::setTasks(hqp_controllers_msgs::SetTasks::Request & req, hqp_controllers_msgs::SetTasks::Response &res)
{
    ...
    std::cout<<"trying to set tasks ..."<<std::endl;
...
//        boost::shared_ptr<std::vector<TaskLink> > t_objs(new std::vector<TaskLink>(n_t_obj));
//        for(unsigned int j=0; j<n_t_obj; j++)
//        {
//            TaskLink t_obj;
//            if(!task_manager_.getTaskLink(task.t_obj_ids[j], t_obj))
//            {
//                res.success = false;
//                lock_.unlock();
